...
from tensorflow.keras.layers import Dense, Flatten, Conv3D, MaxPooling3D, Conv2D, MaxPooling2D, AveragePooling2D
...
if __name__ == "__main__":
    ...
    treningPodaci = pd.read_csv("trening_14_8192_28.csv")  # učitavanje mfcc koeficijenata iz prethodno generisanog csv fajla
    testPodaci = pd.read_csv("testing_14_8192_28.csv")
    ...
    datain = np.zeros([dataSamplesN, 28, 28])
    datainmat = np.array(datainDataFrame)
    for i in range(dataSamplesN):
        datain[i,:,:14] = np.reshape(datainmat[i,:392], (28,14))
    for i in range(dataSamplesN):
        datain[i,:,14:] = np.reshape(datainmat[i,392:], (28,14))

    dataout = np.array(dataoutDataFrame)

    testin = np.zeros([testSamplesN, 28, 28])
    testinmat = np.array(testinDataFrame)
    for i in range(testSamplesN):
        testin[i,:,:14] = np.reshape(testinmat[i,:392], (28,14))
    for i in range(testSamplesN):
        testin[i,:,14:] = np.reshape(testinmat[i,392:], (28,14))
    ...
    # Build the model.
    # model = Sequential([
    #   Conv2D(32, kernel_size=(3, 3, 1), activation='relu', input_shape=(20, 13, 2, 1)),
    #   MaxPooling2D(pool_size=(2, 2, 1)),
    #   Conv3D(16, kernel_size=(3, 3, 1), activation='relu'),
    ...
    for i in range(1,30):
        ...
        spasi = "CNNsadelta_" + str(accuracy) + ".h5"
