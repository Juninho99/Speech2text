...
if __name__ == "__main__":
    ...
    treningPodaci = pd.read_csv("trening_wav_44100.csv")  # učitavanje mfcc koeficijenata iz prethodno generisanog csv fajla
    testPodaci = pd.read_csv("testing_wav_44100.csv")


    datainDataFrame = treningPodaci.loc[ : , treningPodaci.columns != "0"]
    ...
    testinDataFrame = testPodaci.loc[ : , testPodaci.columns != "0"]
    ...
    datain  = np.array(datainDataFrame)
    dataout = np.array(dataoutDataFrame)

    testin  = np.array(testinDataFrame)
    testout = np.array(testoutDataFrame)

    # print(datain.shape, dataout.shape, testin.shape, testout.shape)
    # X_train, X_val, y_train, y_val = train_test_split(datain, dataout, test_size=0.01)
    # print(X_train[0])
    # print(datain[0])
    ...
    # Build the model.
    model = Sequential([
      Conv1D(10, kernel_size=125, input_shape = (44100, 1), strides = 25),
      AveragePooling1D(pool_size=2),
      Conv1D(20, kernel_size=25, strides = 5),
      AveragePooling1D(pool_size=2),
    ...
    for i in range(1,2):
        ...
        # print(X_train[0,0], X_train[0,-1], X_train[-1,0], X_train[-1,-1])
        # print(X_val[0], X_val[-1])
        # print(y_train[0,0], y_train[0,-1], y_train[-1,0], y_train[-1,-1])
